...
export default function NsLookup() {
	...
	const [ Campos, setCampos ] = React.useState({ Tipo: "A", Texto: "" });
	...
	const BtnExecutar = () => {
		...
		if( Campos.Texto == "" ){
			...
			acoes.Executar(Campos.Tipo, Campos.Texto, setProcessado, setFalha, Resultado, setResultado, setResultado2);
						...
						<TableBody>
							{Resultado.map((ret, idx) => (
								<StyledTableRow key={idx}>
									<StyledTableCell>{ret.Nome}</StyledTableCell>
									<StyledTableCell>{ret.Destino}</StyledTableCell>							
									<StyledTableCell>{ret.TTL}</StyledTableCell>
								</StyledTableRow>
							))}	
						</TableBody>
					</Table>
					...
					<Grid item xs={12}>
                        ...
                        <FormControl variant="outlined" fullWidth={true}>
                            <InputLabel>Tipo</InputLabel>
								...
								<MenuItem value={'NAPTR'}>NAPTR</MenuItem>
                            </Select>
                        </FormControl>
