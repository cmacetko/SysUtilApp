...
export default function ConsultarCNPJ() {
	...
	const BtnExecutar = () => {
		...
		if( Campos.CNPJ == "" ){
			...
			.then(result => {
				...
				setResultado({ CNPJ: Campos.CNPJ, Dados: result, Json: funcoes.ArrayToJson(result) });
	...
	const Elemento_Render = (idx, ret) => {

		console.log(idx + " - " + ret);
		
		if( Array.isArray(Resultado.Dados[ret]) )
		{
			...
			var RetValro = Resultado.Dados[ret];
		...
		<Card className={'p10 mt20'}>
			<CardContent>
				...
				<TableContainer component={Paper} className={'mt20'}>
					<Table>
						<TableBody>
							{Object.keys(Resultado.Dados).map((ret, idx) => Elemento_Render(idx, ret))}	
						</TableBody>
					</Table>
				</TableContainer>
