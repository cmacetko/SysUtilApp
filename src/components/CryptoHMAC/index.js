...
export default function CryptoHMAC() {
    
    const { openModAlert } = useModAlert();
	const [ Campos, setCampos ] = React.useState({ Tipo: "MD5", Texto: "", Chave: "" });
    const [ Resultado, setResultado ] = React.useState('');

	React.useEffect(() => {
		
		setResultado(acoes.Executar(Campos.Tipo, Campos.Texto, Campos.Chave));
