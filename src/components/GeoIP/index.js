...
export default function GeoIP() {
	...
	const [ Resultado, setResultado ] = React.useState({ Texto: "", Dados: {}, Json: "" });
	...
	const BtnExecutar = () => {
		...
		if( Campos.Texto == "" ){
			...
			acoes.Executar(Campos.Texto)
			.then(result => {
				
				setGlobalSpinnerState({ open: false });
				setProcessado(true);
				
				setResultado({ Texto: Campos.Texto, Dados: funcoes.ObjectComplexToObjectSimple(result), Json: funcoes.ArrayToJson(result) });
